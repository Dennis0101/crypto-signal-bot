...
export function createRouter() {
  ...
  async function handleCallback(req: any, res: any) {
    ...
    if (!expectedState || !codeVerifier) return res.status(400).send('Invalid OIDC cookie payload');

    // Support both query callbacks (GET) and form_post (POST) for Apple.
    const qp = new URLSearchParams(req.query as any);
    const bp = new URLSearchParams((req.body ?? {}) as any);
    ...
    if (state !== expectedState) return res.status(400).send('State mismatch');
