...
export function createRouter() {
  ...
  r.post('/halt', async (req: AuthedRequest, res) => {
    const userId = req.userId!;
  ...
  r.post('/paper/open', async (req: AuthedRequest, res) => {
    const userId = req.userId!;
  ...
  r.post('/paper/close', async (req: AuthedRequest, res) => {
    const userId = req.userId!;
    const halted = await isUserHalted(userId);
    if (halted.halted) return res.status(423).json({ error: { code: 'HALTED', message: 'Trading halted', reasons: halted.reasons } });
  ...
  r.post('/paper/execute-signal', async (req: AuthedRequest, res) => {
    const userId = req.userId!;
    const halted = await isUserHalted(userId);
    if (halted.halted) return res.status(423).json({ error: { code: 'HALTED', message: 'Trading halted', reasons: halted.reasons } });
