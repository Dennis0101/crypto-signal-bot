...
export function createRouter() {
  ...
  r.get('/', async (req: AuthedRequest, res) => {
    ...
    const rows = await withRlsUser(userId, (tx) =>
  ...
  r.post('/', async (req: AuthedRequest, res) => {
    ...
    if (!v.ok) return res.status(400).json({ error: { code: 'INVALID_KEY', message: v.message } });
    ...
    try {
      const row = await withRlsUser(userId, (tx) =>
        tx.exchangeKey.create({
          data: {
            userId,
