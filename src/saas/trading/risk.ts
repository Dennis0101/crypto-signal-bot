...
export type RiskSnapshot = {
  ...
  realizedLoss24hUsd: number; // positive number representing loss amount
};
...
export async function getRiskSnapshot(userId: string): Promise<RiskSnapshot> {
  return await withRlsUser(userId, async (tx) => {
    const risk =
      (await tx.riskLimit.findUnique({ where: { userId } })) ??
    ...
    const realizedLoss24hUsd = closed.reduce((loss, t) => {
      ...
      if (!pnl) return loss;
      if (pnl.greaterThanOrEqualTo(0)) return loss;
...
export async function assertCanOpenPosition(userId: string, addExposureUsd: number) {
  const snap = await getRiskSnapshot(userId);
  if (!snap.tradingEnabled) {
    return { ok: false as const, code: 'UNCONFIGURED', message: 'Trading is disabled.' };
  }
