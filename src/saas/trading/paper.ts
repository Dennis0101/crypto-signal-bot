...
export async function paperOpenPosition(userId: string, input: PaperOpenInput) {
...
export async function paperClosePosition(userId: string, tradeId: string, idempotencyKey: string) {
  ...
  return await withRlsUser(userId, async (tx) => {
    ...
    if (trade.status !== 'OPEN') return { closed: true, trade };
