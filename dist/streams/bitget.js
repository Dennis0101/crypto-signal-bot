...
let wantSubs = new Set();
/** 외부에서 구독 시작 */
export function subscribeStream(target) {
    const k = keyOf(target.symbol, target.tf);
    wantSubs.add(k);
    if (!state[k])
        state[k] = { lastUse: Date.now(), trades: [], targets: [], lastPushTs: 0 };
...
/** 외부에서 구독 해제(선택) */
export function unsubscribeStream(symbol, tf, predicate) {
...
function handleMessage(buf) {
    try {
        ...
        if (msg.topic?.includes('trade') || msg.arg?.channel === 'trade') {
            const symbol = msg.arg?.instId || parseSymbolFromTopic(msg.topic);
            const trades = (msg.data || []).map(parseTrade).filter(Boolean);
        ...
        if (msg.topic?.includes('candle') || msg.arg?.channel === 'candle') {
            const symbol = msg.arg?.instId || parseSymbolFromTopic(msg.topic);
            const tf = msg.arg?.granularity || parseTFfromTopic(msg.topic);
            const c = parseCandle(msg.data?.[0]);
            if (c)
                onCandle(symbol, tf, c);
        }
    }
    catch { /* ignore */ }
}
...
function parseTFfromTopic(topic) {
...
function parseTrade(row) {
    ...
    if (!isFinite(ts) || !isFinite(price) || !isFinite(size))
        return null;
...
function parseCandle(row) {
    ...
    if (![ts, open, high, low, close].every(isFinite))
        return null;
    return { ts, open, high, low, close };
}
function onTrades(symbol, trades) {
...
async function pushIfNeeded(symbol, tf, force = false) {
    const k = keyOf(symbol, tf);
    const s = state[k];
    if (!s)
        return;
    const now = Date.now();
    if (!force && now - s.lastPushTs < DEBOUNCE_MS)
        return;
    // 분석 파이프라인
...
export function setDiscordClient(c) { globalDiscordClient = c; }
