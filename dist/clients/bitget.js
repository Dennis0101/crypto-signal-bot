...
function toV1FuturesSymbol(sym) {
...
/** ===== Candles ===== */
export async function fetchCandles(symbol, tf, limit = 300) {
...
/** ===== Recent Trades ===== */
export async function fetchRecentTrades(symbol, startMs, endMs, limit = 5000) {
    const toSide = (v) => String(v ?? "").toLowerCase() === "buy" ? "buy" : "sell";
    // 1) v2 fills-history
    try {
        ...
        const jh = await fetchJson(urlHist);
        const rowsH = Array.isArray(jh?.data) ? jh.data : [];
        const outH = rowsH
            .map((t) => ({
    ...
    try {
        ...
        const jr = await fetchJson(urlRecent);
        const rowsR = Array.isArray(jr?.data) ? jr.data : [];
        const outR = rowsR
            .map((t) => ({
            time: Number(t.ts ?? t.time ?? t[3]),
            price: Number(t.price ?? t[0]),
...
/** ===== 랭킹/심볼 ===== */
/** 간단 캐시 (API 과호출 방지) */
const TICKER_CACHE_TTL = CONFIG?.CACHE_TTL_MS ? Number(CONFIG.CACHE_TTL_MS) : 60_000;
...
export async function fetchTicker(symbol) {
    try {
        ...
        const price = toNum(d.last) ??
            toNum(d.lastPrice) ??
            toNum(d.close) ??
            toNum(d.price);
        const raw = d.changeUtc24h ??
            d.changeRatio ??
            d.changePct ??
            d.change ??
            d.pchg;
