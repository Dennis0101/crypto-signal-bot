...
import { getAccount, getPosition, upsertPosition, removePosition, addEquity, setOrderAmount, setLeverage, setCurrency, resetAccount } from './store.js';
import { qtyFromNotionalUSD, unrealizedPnlUSD } from './math.js';
import { getSafePrice } from './price.js';
/** ON/OFF */
export function toggleEnabled(guildId, userId) {
    const acc = getAccount(guildId, userId);
    acc.enabled = !acc.enabled;
    return acc.enabled;
}
/** USD↔KRW */
export function toggleCurrency(guildId, userId) {
    const acc = getAccount(guildId, userId);
...
/** 주문 금액 설정 */
export function setPaperAmount(guildId, userId, usd) {
    setOrderAmount(guildId, userId, usd);
    return getAccount(guildId, userId).orderAmountUSD;
}
/** 레버리지 설정 */
export function setPaperLeverage(guildId, userId, lev) {
    setLeverage(guildId, userId, lev);
    return getAccount(guildId, userId).leverage;
}
export function resetPaper(guildId, userId) {
    resetAccount(guildId, userId);
}
/** 시장가 체결(가상) */
export async function placePaperOrder(guildId, userId, symbol, side) {
    const acc = getAccount(guildId, userId);
    if (!acc.enabled)
        throw new Error('Paper OFF');
...
/** 청산 */
export async function closePaperPosition(guildId, userId, symbol) {
    const pos = getPosition(guildId, userId, symbol);
    if (!pos)
        throw new Error('포지션 없음');
    const price = await getSafePrice(symbol);
    if (!isFinite(price))
        throw new Error('현재가 조회 실패');
...
/** 반전 */
export async function flipPaperPosition(guildId, userId, symbol) {
    const pos = getPosition(guildId, userId, symbol);
    if (!pos)
        throw new Error('포지션 없음');
    await closePaperPosition(guildId, userId, symbol);
