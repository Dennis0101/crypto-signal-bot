...
async function medianCloseFrom1m(symbol, n = 3) {
...
function isOutlier(value, ref, thresholdPct = 0.15) {
    if (!isFiniteNum(value) || !isFiniteNum(ref) || ref <= 0)
        return true;
    const dev = Math.abs(value - ref) / ref;
    return dev > thresholdPct;
}
/** 안전한 현재가(USDT 선물) — 티커 → 폴백(1m 중앙값) */
export async function getSafePrice(symbol) {
    const key = `p:${symbol}`;
...
export async function getSafeTicker(symbol) {
    const key = `t:${symbol}`;
    const c = cache.get(key);
    if (c && now() - c.at < CACHE_TTL_MS && isFiniteNum(c.price)) {
